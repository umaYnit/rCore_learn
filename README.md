...
lb/lh/lw/ld/ t0, 8(sp)
	loads from (sp + 8) into register t0.
	lb = load byte, lh = load halfword, lw = load word, ld = load doubleword.

sb/sh/sw/sd/ t0, 8(sp)
	stores from register t0 into (sp + 8).
...
auipc rd, immediate
	...
	把符号位扩展的20位（左移12位）立即数加到pc上，结果写入x[rd]。
...
2. 请学习 gdb 调试工具的使用(这对后续调试很重要)，并通过 gdb 简单跟踪从机器加电到跳转到 0x80200000 的简单过程。只需要描述重要的跳转即可，只需要描述在 qemu 上的情况。
    ...
    >   - `continue`: 执行直到碰到断点。
    >- `si`: 单步执行一条汇编指令。
    >   - `set disassemble-next-line on`：`si`时自动反汇编
...
1. 正确进入 U 态后，程序的特征还应有：使用 S 态特权指令，访问 S 态寄存器后会报错。目前由于一些其他原因，这些问题不太好测试，请同学们可以自行测试这些内容（参考 [前三个测例](https://github.com/DeathWish5/rCore_tutorial_tests/tree/master/user/src/bin) )，描述程序出错行为，同时注意注明你使用的 sbi 及其版本。
   ...
   答：使用之前版本的Rustsbi会直接在m态就崩溃掉，并且无法退出程序。切换到opensbi后，能够正确的捕获到错误并分发。
...
2. 请结合用例理解 [trap.S](https://github.com/rcore-os/rCore-Tutorial-v3/blob/ch2/os/src/trap/trap.S) 中两个函数 `__alltraps` 和 `__restore` 的作用，并回答如下几个问题:
   ...
   1. L40: 刚进入 `__restore` 时，`a0` 代表了什么值。请指出 `__restore` 的两种使用情景。
      ...
      答：先说`__restore`使用的两个场景：

      一是在`__alltraps`调用完`trap_handler`方法后直接向下继续执行到的。而这里又是什么时候执行的`__alltraps`呢，噗，，就如名字一样，在处理*trap*时被调用（`__alltraps`的地址是在在`main`方法中调用`trap::init()`时被写入到`stvec`寄存器）。因此这里`__restore`的作用就是处理*trap*后返回U态。

      第二呢，就是在`batch.rs`里的`run_next_app`方法里直接调用的。这里的用途就很明显啦，就是启动运行APP。

      接着就是`a0`的值了，`a0`寄存器代表的是函数参数，可以看到，无论是场景一还是场景二，都是`TrapContext`的指针，之所以这样做是因为两个场景时的`sp`不一样（想要复用`__restore`函数）。
   ...
   2. L46-L51: 这几行汇编代码特殊处理了哪些寄存器？这些寄存器的的值对于进入用户态有何意义？请分别解释。
      ...
      答：前三行为将值读取到t0-t2临时寄存器，用于协助后面将值读取到*csr*寄存器。这几行代码主要作用是处理与*trap*功能相关的寄存器`sstatus`、`sepc`和`sscratch`，将保存的值从*kernel*栈上读取出来。

      `sstatus`寄存器记录了进入*trap*前的特权等级，在最后调用`sret`返回时，会根据该寄存器里的字段设置特权级；`spec`寄存器记录的是返回后执行的指令的地址；而`sscratch`寄存器记录的是返回后的栈指针`sp`。
   ...
   3. L53-L59: 为何跳过了 `x2` 和 `x4`？
      ...
      答：x4寄存器保存线程指针，所以这里不需要处理；x2寄存器即sp寄存器，保存栈指针，在后面处理了。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           
   ...
   4. L63: 该指令之后，`sp` 和 `sscratch` 中的值分别有什么意义？
      ...
      答：该指令交换了`sp`寄存器和`sscratch`寄存器的值。该指令后`sp`的值指向用户栈，`sscratch`的值为进入*Trap*之前的状态并指向内核栈。

   5. `__restore`：中发生状态切换在哪一条指令？为何该指令执行之后会进入用户态？

      答：发生状态切换在`sret`指令。在调用`sret`指令时，CPU会根据`sstatus`的`SPP`字段的值设置特权级，而`sstatus`在这里为U，所以进入了用户态。
   ...
   6. L13： 该指令之后，`sp` 和 `sscratch` 中的值分别有什么意义？
      ...
      答：该指令交换了`sp`寄存器和`sscratch`寄存器的值。该指令后，`sp`里的值为内核的栈指针地址，`sscratch`的值为用户栈的指针地址。

   7. 从 U 态进入 S 态是哪一条指令发生的？

      答：一般情况下是应用程序通过`ecall`指令向操作系统请求服务时，完成的切换。

3. 描述程序陷入内核的两大原因是中断和异常，请问 riscv64 支持哪些中断／异常？如何判断进入内核是由于中断还是异常？描述陷入内核时的几个重要寄存器及其值。

   答：支持的中断和异常我这里取巧，直接查看在处理时调用的riscv库里查看，在`scause.cause()`后，返回一个枚举`Trap`，该枚举有两个枚举值：Interrupt和Exception，它们又个是一个枚举，值分别为：
   
   - `Interrupt`：`UserSoft`, `SupervisorSoft`, `UserTimer`, `SupervisorTimer`, `UserExternal`, `SupervisorExternal`, `Unknown`
   
   - `Exception` `InstructionMisaligned`, `InstructionFault`, `IllegalInstruction`, `Breakpoint`, `LoadFault`, `StoreMisaligned`, `StoreFault`, `UserEnvCall`, `InstructionPageFault`, `LoadPageFault`, `StorePageFault`, `Unknown`
   
     陷入内核时的重要寄存器在上面问题基本已经回答过了，故不再赘述。
   
4. 对于任何中断， `__alltraps` 中都需要保存所有寄存器吗？你有没有想到一些加速 `__alltraps` 的方法？简单描述你的想法。

   答：不需要。加速`__alltraps`的话，可以预先判断是什么*trap*，再根据具体的情况只保存用到的寄存器。
